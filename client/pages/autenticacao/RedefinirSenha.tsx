...
export default function RedefinirSenha() {
  ...
  // Best effort: listen to PASSWORD_RECOVERY event to ensure session is present
  useEffect(() => {
    const { data: sub } = supabase.auth.onAuthStateChange((_event) => {
      // No-op: presence of event ensures SDK parsed recovery tokens
    });
    return () => {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    if (senha1 !== senha2) {
      toast({ title: "Senhas não conferem", description: "Digite a mesma senha nos dois campos." });
      return;
    }
    setLoading(true);
    try {
      ...
      if (!sessionData?.session) {
        toast({ title: "Link inválido ou expirado", description: "Abra o link do e-mail novamente para redefinir a senha." });
        return;
      }
      const { error } = await supabase.auth.updateUser({ password: senha1 });
      if (error) {
        toast({ title: "Erro ao salvar senha", description: error.message });
        return;
      }
      toast({ title: "Senha alterada", description: "Use sua nova senha para acessar." });
        ...
        <div className="w-full max-w-sm sm:max-w-md md:max-w-lg">
          <div className="rounded-[10px] border border-white bg-white shadow-[0_0_2px_0_rgba(23,26,31,0.12),0_0_1px_0_rgba(23,26,31,0.07)] p-6 sm:p-8 xl:p-10 max-[360px]:p-4">
            ...
            <div className="mb-6 text-center sm:mb-8 max-[360px]:mb-5">
              <h2 className="mb-2 font-open-sans text-xl font-bold text-sis-dark-text sm:mb-3 sm:text-2xl xl:text-[30px] xl:leading-9 max-[360px]:text-lg">Redefinir Senha</h2>
              <p className="font-roboto text-sm text-sis-secondary-text sm:text-base xl:leading-6 max-[360px]:text-xs">Digite sua nova senha abaixo.</p>
            </div>

            <form onSubmit={onSubmit} className="space-y-6 max-[360px]:space-y-5">
              <div className="space-y-3.5">
                <label className="block font-roboto text-xs font-medium text-sis-dark-text xl:leading-5">Nova senha</label>
                <input
                  type="password"
              ...
              <div className="space-y-3.5">
                <label className="block font-roboto text-xs font-medium text-sis-dark-text xl:leading-5">Confirmar nova senha</label>
                <input
                  type="password"
                  autoComplete="new-password"
                  required
