...
type MetricDefinition = {
  key: "total" | "analise" | "acordos" | "conformidade";
  titulo: string;
  descricao: string;
  icon: ReactNode;
};

type Metric = MetricDefinition & { valor: string };
...
function buildDefaultMetrics(): Metric[] {
  return metricDefinitions.map((def) => ({
    ...def,
    valor: def.key === "conformidade" ? "0%" : "0",
  }));
}
...
export default function JuridicoDashboard() {
  ...
  const [metricas, setMetricas] = useState<Metric[]>(() => buildDefaultMetrics());
  ...
  useEffect(() => {
    ...
    fetchProcesses()
      .then((data) => {
        ...
        const mapped: ProcessoJuridico[] = (data || []).map((p: any) => ({
          id: p.id,
        ...
        const conformidadeRatio = total > 0 ? finalizadosCount / total : 0;

        setMetricas(
          metricDefinitions.map((def) => {
            switch (def.key) {
              case "total":
                return { ...def, valor: numberFormatter.format(total) };
              case "analise":
                return { ...def, valor: numberFormatter.format(emAnaliseCount) };
              case "acordos":
                return { ...def, valor: numberFormatter.format(acordosEffective) };
              case "conformidade":
                return { ...def, valor: percentFormatter.format(conformidadeRatio) };
            }
          })
        );
      })
      .catch(() => {
        if (!mounted) return;
        setProcessos([]);
        setMetricas(buildDefaultMetrics());
      })
      .finally(() => {
        if (mounted) setLoadingProcessos(false);
      });
